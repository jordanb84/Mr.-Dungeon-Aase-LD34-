...
public class EntityFireGuard extends EntityEnemy{

	private float elapsed;
	
	public EntityFireGuard(Vector2 location) {
		super(new EntityAnimation(new Texture(Gdx.files.internal("assets/texture/entity/firebeastSheet.png")), .3f), location, 2, 5, true);
		//this.setSpeed(0.1f);
	...
	public void update() {
		...
		if(elapsed > .8f){
			EntityBullet bullet0 = (new EntityBullet(new Vector2(this.getLocation().x, this.getLocation().y - 64), Color.FIREBRICK, EntityDirection.LEFT, 200, false, 1f, true, false, true));
			EntityBullet bullet1 = (new EntityBullet(new Vector2(this.getLocation().x, this.getLocation().y - 64), Color.FIREBRICK, EntityDirection.RIGHT, 200, false, 1f, true, false, true));
		...
		if(this.getLight() == null){
			this.setLight(new PointLight(this.getMap().getRay(), Map.RAYS, Color.FIREBRICK, 64, this.getLocation().x, this.getLocation().y));
		}
