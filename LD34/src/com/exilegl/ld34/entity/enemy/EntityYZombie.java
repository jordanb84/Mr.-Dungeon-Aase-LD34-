...
public class EntityYZombie extends EntityEnemy{

	private float elapsed;
	
	public EntityYZombie(Vector2 location) {
		super(new EntityAnimation(new Texture(Gdx.files.internal("assets/texture/entity/demonSheet.png")), .3f), location, 2, 5, true);
		//this.setSpeed(0.1f);
	...
	public void update() {
		...
		if(elapsed > .8f){
			this.getMap().spawnEntity(new EntityBullet(new Vector2(this.getLocation().x, this.getLocation().y), Color.RED, EntityDirection.LEFT, 200, false, 1f, false, false, false));
			this.getMap().spawnEntity(new EntityBullet(new Vector2(this.getLocation().x, this.getLocation().y), Color.RED, EntityDirection.RIGHT, 200, false, 1f, false, false, false));
			elapsed = 0;
		}
