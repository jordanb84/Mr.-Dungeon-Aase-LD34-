...
public class EntityWaterBeast extends EntityEnemy{

	private float elapsed;
	
	public EntityWaterBeast(Vector2 location) {
		super(new EntityAnimation(new Texture(Gdx.files.internal("assets/texture/entity/waterbeastSheet.png")), .3f), location, 2, 5, true);
		//this.setSpeed(0.1f);
		
		this.getAnimation().addDirectionFrame(EntityDirection.LEFT, 0, 0, 128, 128);
		this.getAnimation().addDirectionFrame(EntityDirection.LEFT, 0, 0, 128, 128);
		this.getAnimation().addDirectionFrame(EntityDirection.RIGHT, 0, 0, 128, 128);
		this.getAnimation().addDirectionFrame(EntityDirection.RIGHT, 0, 0, 128, 128);
		this.getAnimation().addDirectionFrame(EntityDirection.UP, 0, 0, 128, 128);
		this.getAnimation().addDirectionFrame(EntityDirection.UP, 0, 0, 128, 128);
		this.getAnimation().addDirectionFrame(EntityDirection.DOWN, 0, 0, 128, 128);
		this.getAnimation().addDirectionFrame(EntityDirection.DOWN, 0, 0, 128, 128);
	...
	public void update() {
		...
		if(elapsed > .05f){
			...
			EntityBullet bullet2 = (new EntityBullet(new Vector2(this.getLocation().x - 100, this.getLocation().y - 64), Color.CYAN, EntityDirection.LEFT, 200, false, 1f, true, false, false));
			EntityBullet bullet3 = (new EntityBullet(new Vector2(this.getLocation().x + 100, this.getLocation().y - 64), Color.CYAN, EntityDirection.RIGHT, 200, false, 1f, true, false, false));
			bullet0.setTexture(new Texture(Gdx.files.internal("assets/texture/misc/snowball.png")));
			bullet1.setTexture(new Texture(Gdx.files.internal("assets/texture/misc/snowball.png")));
			bullet2.setTexture(new Texture(Gdx.files.internal("assets/texture/misc/waterball.png")));
			bullet3.setTexture(new Texture(Gdx.files.internal("assets/texture/misc/waterball.png")));
			this.getMap().spawnEntity(bullet0);
			this.getMap().spawnEntity(bullet1);
			this.getMap().spawnEntity(bullet2);
			this.getMap().spawnEntity(bullet3);
			elapsed = 0;
		}
