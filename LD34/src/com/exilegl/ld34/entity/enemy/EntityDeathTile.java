...
public class EntityDeathTile extends EntityEnemy{

	private float elapsed;
	
	private float hp;
	
	public EntityDeathTile(Vector2 location) {
		super(null, location, 2, 5, true);
		//this.setSpeed(0.1f);
		
		this.setSingleAnimation(new Animation());
		this.getSingleAnimation().addFrame(new Frame(new TextureRegion(new Texture(Gdx.files.internal("assets/texture/tile/furnace.png"))), 1));
	...
	public void update() {
		...
		if(this.getLight() == null){
			this.setLight(new PointLight(this.getMap().getRay(), Map.RAYS, Color.PURPLE, 84, this.getLocation().x, this.getLocation().y));
		}
