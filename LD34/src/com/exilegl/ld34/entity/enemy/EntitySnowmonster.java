...
public class EntitySnowmonster extends EntityEnemy{

	private float elapsed;
	
	public EntitySnowmonster(Vector2 location) {
		super(new EntityAnimation(new Texture(Gdx.files.internal("assets/texture/entity/snowman.png")), .3f), location, 2, 5, true);
		//this.setSpeed(0.1f);
	...
	public void update() {
		...
		if(elapsed > .8f){
			EntityBullet bullet0 = (new EntityBullet(new Vector2(this.getLocation().x, this.getLocation().y - 64), Color.WHITE, EntityDirection.LEFT, 200, false, 1f, true, false, true));
			EntityBullet bullet1 = (new EntityBullet(new Vector2(this.getLocation().x, this.getLocation().y - 64), Color.WHITE, EntityDirection.RIGHT, 200, false, 1f, true, false, true));
		...
		if(this.getLight() == null){
			this.setLight(new PointLight(this.getMap().getRay(), Map.RAYS, Color.WHITE, 64, this.getLocation().x, this.getLocation().y));
		}
...
//it will shoot snowballs at you