...
public class EntitySlowMadman extends EntityEnemy{

	private float elapsed;
	
	public EntitySlowMadman(Vector2 location) {
		super(new EntityAnimation(new Texture(Gdx.files.internal("assets/texture/entity/madmanSheet.png")), .3f), location, 2, 5, true);
		//this.setSpeed(0.1f);
	...
	public void update() {
		...
		if(elapsed > .1f){
			this.getMap().spawnEntity(new EntityBullet(new Vector2(this.getLocation().x, this.getLocation().y), Color.GREEN, EntityDirection.LEFT, 200, false, 1f, false, false, false));
			this.getMap().spawnEntity(new EntityBullet(new Vector2(this.getLocation().x, this.getLocation().y), Color.GREEN, EntityDirection.RIGHT, 200, false, 1f, false, false, false));
			elapsed = 0;
		}
		
		if(this.getLight() == null){
			this.setLight(new PointLight(this.getMap().getRay(), Map.RAYS, Color.FOREST, 64, this.getLocation().x, this.getLocation().y));
		}
