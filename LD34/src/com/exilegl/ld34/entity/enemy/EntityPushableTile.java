...
public class EntityPushableTile extends EntityEnemy{

	private float elapsed;
	
	private float hp;
	
	public EntityPushableTile(Vector2 location) {
		super(null, location, 2, 5, true);
		//this.setSpeed(0.1f);
		
		this.setSingleAnimation(new Animation());
		this.getSingleAnimation().addFrame(new Frame(new TextureRegion(new Texture(Gdx.files.internal("assets/texture/tile/brick.png"))), 1));
	...
	public void update() {
		...
		if(this.getLight() == null){
			this.setLight(new PointLight(this.getMap().getRay(), Map.RAYS, Color.LIME, 64, this.getLocation().x, this.getLocation().y));
		}
		...
		for(Entity e : this.getMap().getEntities()){
			if(e instanceof EntityEnemy && !(e instanceof EntityPlayer) && !(e instanceof EntityBullet) && !(e instanceof EntityPushableTile)){
				try{
				if(e.getRectangle().overlaps(this.getRectangle()) && !e.isDead() && !this.isDead()){
					e.kill();
					this.kill();
